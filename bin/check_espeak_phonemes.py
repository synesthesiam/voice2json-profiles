...
def main():
    ...
    with open(args.examples, "r") as examples_file:
        for example_line in examples_file:
            ...
            dict_phoneme, word, dict_phoneme_str = re.split(
                r"\s+", example_line, maxsplit=2
            )
            dict_phonemes = re.split(r"\s+", dict_phoneme_str)
            ...
            while True:
                ...
                with open(args.map, "r") as map_file:
                    for map_line in map_file:
                        ...
                        map_dict_phoneme, map_espeak_phoneme = re.split(
                            r"\s+", map_line
                        )
                        phoneme_map[map_dict_phoneme] = map_espeak_phoneme

                # Speak expected
                print(dict_phoneme, word, dict_phoneme_str, "expected")
                espeak_cmd = ["espeak-ng", "-x", "-s", "80"]
                ...
                # Speak actual
                actual_espeak = [phoneme_map.get(p, p) for p in dict_phonemes]
                actual_espeak_str = "".join(actual_espeak)

                print(dict_phoneme, word, dict_phoneme_str, "actual")
                espeak_cmd = ["espeak-ng", "-x", "-s", "80"]
                if args.voice is not None:
                    espeak_cmd.extend(["-v", args.voice])
